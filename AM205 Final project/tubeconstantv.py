...
"""runs the calcualtion for n descrete spatial steps, and nt descrete time steps and plots the pipe at t=14.1 sec and tfin
"""
def calctube(outertemp):
 ...
 #Generic matrix entities
            ...
            M[j  +(nr+2)*i,j-1  +(nr+2)*i]=-aw*dt/dr*(1./dr - 1./rad[j]/2.)
            M[j  +(nr+2)*i,j+((nr+2))*(i+1)] = -aw*dt*(1./dz/dz) + vm/2*dt/dz
            M[j  +(nr+2)*i,j+((nr+2))*(i-1)] = -aw*dt*(1./dz/dz) - vm/2*dt/dz
...
#r=0 for  z 1:nz+1
        ...
        M[(nr+2)*i,((nr+2))*(i+1)] = -aw*dt*(1./dz/dz) + vm/2*dt/dz
        M[(nr+2)*i,((nr+2))*(i-1)] = -aw*dt*(1./dz/dz) - vm/2*dt/dz
...
#r=OD    
    ...
    ##this plots the tube at t= tfin and t = 14.1 seconds.
    fig = plt.figure()
