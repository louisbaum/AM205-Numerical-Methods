...
#thermal constants (density and specific heat)
cpwater = cp(323.15)[0]
pwater = p(323.15)[0]
cpwort =cp(323.15)[1]
pwort = p(323.15)[1]
...
tapwater = 288.7 #K
KettleVol = 0.0189271 #m^3 = 5 gallons volume to wort bath
heat = numpy.zeros([nt])
for j in range(copperbound):
    vr[j]= 2.*vm*(1.-rad[j]**2/(ID/2.)**2)#laminer flow
...
#r=OD    
    ...
    for n in range(nt-1):
        ...
        for j in range(0,copperbound):
            heat[n+1] = heat[n+1] + (pipe[j,nz+1,n+1]-288.7)*dt*vr[j]*dr*numpy.pi*2*rad[j]*cpwater*pwater
        for i in range(nz+1):
...
"""plots the temperature at two different times tfin and 14.1 seconds
"""    
    
def plotpipe():   
    fig = plt.figure()
    a= fig.add_subplot(2,1,1)
    a.set_title('After Interference Effects ('+str(times[nt-1])+' sec)]')
    a=plt.imshow(picpipe[:,:,nt-1]-T0,clim=[0,100],aspect = .15)  
    plt.xticks([0,nz-1],[0,zfin])
    plt.yticks([0,nr],["OD",0])
        
    b = fig.add_subplot(2,1,2)
    b.set_title('Before Interference Effects (' +str(times[423])+ ' sec )')
